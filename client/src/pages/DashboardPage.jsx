...
const DashboardPage = () => {
  ...
  // --- CHANGED: Added state to track the specific ID of the copied row link ---
  ...
  // --- CHANGED: Updated handleCopy to accept the link's unique _id ---
  const handleCopy = async (text, linkId) => {
    try {
      await navigator.clipboard.writeText(text);
      
      // Set the state to the ID of the link we just clicked
      setCopiedLinkId(linkId);

      // Reset the text back to "Copy" after exactly 2 seconds
      setTimeout(() => {
